...
function setVccBarOnEvent() {
  ...
  if (application.oJBarDisplayer != null) application.oJBarDisplayer.show(1);

  showLog(
    application.oBrowserSys.expression + "\r\nevent bindType:[on]\r\n\r\n"
  );
  displayCtrl();
...
function SetLogType() {
  var TempLogFlag = 0;
  if (IsCheckBoxSelected("log_error")) TempLogFlag = TempLogFlag | 1;
  if (IsCheckBoxSelected("log_warn")) TempLogFlag = TempLogFlag | 2;
  if (IsCheckBoxSelected("log_info")) TempLogFlag = TempLogFlag | 4;
  if (IsCheckBoxSelected("log_debug")) TempLogFlag = TempLogFlag | 8;
  if (IsCheckBoxSelected("log_protocal")) TempLogFlag = TempLogFlag | 16;
  var ConsoleLogFlag = 0;
  if (IsCheckBoxSelected("log_console")) ConsoleLogFlag = 1;
...
function displayCtrl() {
  if (getLocalLanguage() != lg_zhcn) {
    ...
    showLog(
      "init success,Version:" + application.oJVccBar.GetVersion() + "\r\n"
    );
  } else {
    showLog("当前使用【PureJS】版本电话条!\r\n");
    showLog(
      "初始化就绪,版本号码:" + application.oJVccBar.GetVersion() + "\r\n"
    );
  }
}
...
function showLog(Text) {
  ...
  if (oTextareaInfo != null) oTextareaInfo.value = oTextareaInfo.value + Text;
  //application.Info(Text);
}
function emptyLog() {
  var oTextareaInfo = document.getElementById("TextareaInfo");
  if (oTextareaInfo != null) oTextareaInfo.value = "";
}
...
function onOnCallEnd(
  ...
  if (Tdelay != null) Tdelay.innerHTML = "延时:0ms";
}

//提示事件
function onOnPrompt(code, description) {
  if (
    application.oJVccBar.GetAttribute("SelfPrompt") == 1 &&
...
function onReportBtnStatus(btnIDS) {
  ...
  if (application.oVccBarAssist.oBarAgentStatus != null) {
    application.oVccBarAssist.oBarAgentStatus.SetAgentStatus(
      application.oJVccBar.GetAgentStatus()
    );
  }
  if (application.oJBarDisplayer != null) {
    application.oJBarDisplayer.ChangeBtnStatus(btnIDS);
    application.oJBarDisplayer.SetAgentStatus(
      application.oJVccBar.GetAgentStatus()
    );
  }
  ...
  if (agentStatus == 1) {
    if (getLocalLanguage() != lg_zhcn)
      agentStatus =
        agentStatus +
        " agent subStatus:【" +
        application.oJVccBar.GetAgentSubBusyStatus() +
        "】";
    else
      agentStatus =
        agentStatus +
...
function onOnInitalSuccess() {
  if (application.oVccBarAssist.oBarAgentStatus != null) {
    application.oVccBarAssist.oBarAgentStatus.SetSubBusyStatus(
      application.oJVccBar.GetBusySubStatus()
    );
  }
  if (application.oJBarDisplayer != null)
  ...
  if (getLocalLanguage() != lg_zhcn)
    showLog(
      "        used phoneType:【" +
        application.oJVccBar.GetAttribute("PhonType") +
  ...
  showLog(
    " AgentName:" + application.oJVccBar.GetAttribute("AgentName") + "\r\n "
  );
  ...
  else showLog(" current server:备服务器\r\n ");
  showLog(
    " *******************************************************************\r\n"
  );
  SaveDefaultParam();
...
function InitMonitorData() {
  if (application.oJMonitorData != null) {
    setTimeout(function () {
      application.oJMonitorData.Initial();
      showLog("【初始化监控】\r\n");
    }, 3 * 1000);
  }
}
...
function onOnAgentLogUpload(
  ...
  var bUpload = false;
  if (urlType == 1 && code == "1" && des == "success") bUpload = true;
  ...
  if (bUpload) {
    ...
    var fullFilePath = stringFormat(
      "http://{0}:9999/media/log/{1}/{2}",
      serverIP,
      vccid,
      fileName
    );
    if (urlType == 0)
      fullFilePath = stringFormat(
        "http://{0}:9999/media/000000/{1}",
        serverIP,
        fileName
      );
    if (getLocalLanguage() != lg_zhcn)
      showLog(" upload file success,directory:[" + fullFilePath + "]\r\n");
    else showLog(" 上传成功,文件目录:【" + fullFilePath + "】\r\n");
  } else {
    if (getLocalLanguage() != lg_zhcn)
      showLog(" upload file failure:code:[" + code + "] des:[" + des + "]\r\n");
    else
      showLog(
        " 上传失败错误信息:code:【" + code + "】 des:【" + des + "】\r\n"
      );
  }
  showLog(
    " *******************************************************************\r\n"
...
function onOnStopCaptureSuccess(serverName, fileName) {
  ...
  showLog(
    " serverName:【" + serverName + "】 fileName:【" + fileName + "】\r\n"
  );
  showLog(
    " *******************************************************************\r\n"
  );
}
function onOnCallCdrReport(
  callid,
  sid,
  usernumber,
  direction,
...
function onOnRtpStatics(stat) {
  if (stat.key_word == "RTPStatics") {
    ...
    if (stat.param.video) {
      ...
      } else $("ul li").css("background", "limegreen");
    } else {
      if (Tdelay != null)
...
function onOnRecvWeChatMessage(
  sessionId,
  msgseq,
  type,
  userId,
...
function onOnSendWeChatMsgReport(
  userId,
  sessionId,
  msgseq,
  code,
...
function OnRecvMessage(sessionId, msgseq) {
  showLog(
    "接收对方消息：【OnRecvMessage(" + sessionId + "," + msgseq + ")】：\r\n"
  );
  g_msgseq = msgseq;
...
function OnSendMessageReport(sessionId, msgseq) {
  showLog(
    "发送消息：【OnSendMessageReport(" + sessionId + "," + msgseq + ")】：\r\n"
  );
  g_msgseq = msgseq;
  var oSession = application.oWechatManager.GetSessionItem(sessionId, msgseq);
  if (oSession != null) {
...
function DisplayDiv(id, flag) {
  ...
  if (oDiv == null) return;
  if (flag == 0) oDiv.style.display = "none";
...
function shareSreen(srcType, plType) {
  ...
  if (srcType == "background") {
    ...
    options.force = true;
    if (plType == 2) options.videosource = "scripts/ui/images/test.png";
    ...
    if (oCameraDevices.options.length > 0) {
      options.videosource =
        oCameraDevices.options[oCameraDevices.options.selectedIndex].value;
    }
